...
class spring_function(pychrono.ForceFunctorP):
    ...
    def __call__(self, d_time, d_rest_length, d_length, d_vel, o_spring_object):
        ...
        d_spring_constant = 400.0 # [N/m] Spring force after initial tolerance



        d_force = -d_spring_constant * (d_length)
...
tank.setSponge(x_n=2*wavelength, x_p=2*wavelength)
...
free_x = np.array([1., 0., 0.]) # translational
...
# create spring attachment
spring_attach = pychrono.ChVectorD(wavelength, water_level, 0.)
springAttachment = fsi.ProtChBody(system=system)                    # Create the body

springAttachment.ChBody.SetPos(spring_attach)                 # Translate the body to the pin location
springAttachment.ChBody.SetBodyFixed(True)    # Set the the body freedom
springAttachment.ChBody.SetMass(0.0)                # Set the guillotine mass
#springAttachment.ChBody.SetInertiaXX(o_inertia_guillotine)        # Set the guillotine inertia
springAttachment.setRecordValues(all_values=True)                 # Log the position


TSDA = pychrono.ChLinkTSDA()
body1_point = pychrono.ChVectorD(wavelength+spring_distance,water_level,0.0)
body2_point = pychrono.ChVectorD(wavelength,water_level,0.0)


TSDA.Initialize(system.subcomponents[0].ChBody,
                                    system.subcomponents[1].ChBody,
                                    False, body1_point, body2_point, auto_rest_length = False, rest_length=0.0)
